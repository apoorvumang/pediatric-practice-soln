...
if(isset($_GET['id']))
{
	...
	$query = "SELECT vaccines.name as vac, vac_schedule.date, patients.name
				FROM patients, vac_schedule, vaccines 
				WHERE patients.id = vac_schedule.p_id AND vac_schedule.v_id = vaccines.id AND patients.id = {$_GET['id']}";
	echo $query;
