...
if($_POST['specificdate']||$_POST['tofromdate']||$_POST['patientsearch'])	//If some submit button clicked
{
	if($_POST['specificdate'])
	{
        ...
        $query = "SELECT a.id as aid, p.id as pid, p.name as pname, a.date as date, a.time as time, a.comment as comment, p.phone as phone, p.phone2 as phone2 FROM patients p, appointments a WHERE a.date ='{$_POST['date']}' AND p.id = a.p_id ORDER BY date, time";
